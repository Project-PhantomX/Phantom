...
minetest.register_on_update_frame(function(dtime) (function(dtime)
	if not minetest.localplayer then return end
	etime = etime + dtime
	if autoeat.lock or minetest.settings:get_bool("autoeat") and autoeat.get_hunger() < get_float("hunger", 9) then
		etime = 0
		autoeat.eat()
	end
end)
